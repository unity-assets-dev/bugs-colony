...
namespace PrimeTween {
    ...
    internal class PrimeTweenManager : MonoBehaviour {
            ...
            get {
                if (!HasInstance) {
                    if (Application.isEditor && !Application.isPlaying) {
                        CreateInstance();
                        Selection.selectionChanged += _instance.ResetCurrentTweenAnimations;
                ...
                } /*else if (_instance == null) { // p2 todo this throws if PrimeTween API is called from OnDestroy(). See the DestructionOrderTest scene. How to detect manual PrimeTweenManager destruction?
        ...
        internal Dictionary<(Transform, TweenType), (TypeUnion startValue, int count)> shakes;
        ...
        static GameObject CreateNewGameObject() {
            ...
            if (Application.isEditor) {
                ...
                if (!EditorApplication.isPlaying && _playModeState != PlayModeStateChange.EnteredEditMode) {
                    DebugLifetime("EnteredEditMode check failed");
                    throw new Exception(exceptionMessage);
                }
                if (EditorApplication.isUpdating) {
                    DebugLifetime("EditorApplication.isUpdating check failed");
                    throw new Exception(exceptionMessage);
                }
                try {
                    ...
                    if (e.Message.Contains("is not allowed to be called from a MonoBehaviour constructor")) {
                        DebugLifetime("new GameObject check failed");
                        throw new Exception(exceptionMessage);
                    }
                    throw;
                }
        ...
        void init(int capacity) {
            ...
            shakes = new Dictionary<(Transform, TweenType), (TypeUnion, int)>(capacity);
        ...
        #if UNITY_EDITOR
        [SerializeField] internal List<TweenAnimation> currentTweenAnimations = new List<TweenAnimation>(); // should be non-readonly list to support script hot reloading
        void ResetCurrentTweenAnimations() {
            #if PRIME_TWEEN_PRO
            DebugLifetime($"ResetCurrentTweenAnimations {_hashCode}");
        ...
        PrimeTweenManager() {
            ...
            DebugLifetime($"PrimeTweenManager {_hashCode}");
        }
        ~PrimeTweenManager() => DebugLifetime($"~PrimeTweenManager {_hashCode}");
        void OnDestroy() => DebugLifetime("OnDestroy");*/

        [Conditional("_")]
        static void DebugLifetime(string log) {
            // Debug.Log($"{_instance?._hashCode}: {log}");
        }

        static PlayModeStateChange _playModeState = PlayModeStateChange.EnteredEditMode;
        ...
        static void iniOnLoad() {
            ...
            EditorApplication.playModeStateChanged += state => {
                ...
                DebugLifetime($"playModeStateChanged: {state}");
                switch (state) {
                    ...
                    case PlayModeStateChange.ExitingEditMode:
                        if (HasInstance) {
                            DebugLifetime($"DestroyImmediate");
            ...
            Selection.selectionChanged += foundInScene.ResetCurrentTweenAnimations;
        }
        ...
        void update(List<ReusableTween> tweens, float deltaTime, float unscaledDeltaTime, out int processedCount, int? maxCount = null) {
            if (updateDepth != 0) {
                CompleteAndClear(this.tweens);
                CompleteAndClear(lateUpdateTweens);
                CompleteAndClear(fixedUpdateTweens);

                void CompleteAndClear(List<ReusableTween> l) {
                    for (int i = 0; i < l.Count; i++) {
                        var t = l[i];
                        if (t != null && t._isAlive) {
