    ...
    public
        ...
        public float duration {
            ...
            private set {
                ...
                rootTween.calculateCycleDuration();
                rootTween.endValue.single = value;
                rootTween.cacheDiff();
            }
        }
        ...
        public static Sequence Create(int cycles = 1, SequenceCycleMode cycleMode = SequenceCycleMode.Restart, Ease sequenceEase = Ease.Linear, bool useUnscaledTime = false, UpdateType updateType = default) {
            ...
            if (sequenceEase == Ease.Default) { // p1 todo this is questionable because passing Default requires an explicit effort from the user. Replace with PrimeTweenManager.defaultEase?
                sequenceEase = Ease.Linear;
            }
            ...
            root.tween.startValue.Reset();
            root.tween.endValue.Reset();
        ...
        public Sequence Insert(float atTime, Tween tween) {
            ...
            if (tween.tween.sequence.IsCreated) {
                Debug.LogError($"{Constants.nestTwiceError} Tween: {tween.tween.GetDescription()}");
                return this;
            }
            setSequence(tween);
        ...
        public void Complete() {
            if (isAlive && tryManipulate()) {
                ...
                root.isPaused = false;
                Assert.IsTrue(root.tween.isMainSequenceRoot());
                root.tween.updateSequence(float.MaxValue, false, true, false, false);
                Assert.IsFalse(isAlive);
            }
        }
        ...
        static void releaseTween([NotNull] ReusableTween tween) {
            ...
            if (tween.isSequenceRoot()) {
                tween.tweenType = TweenType.Disabled;
        ...
        internal struct SequenceDirectEnumerator {
            ...
            public
                ...
                readonly
                #endif
                Tween Current { // p2 todo return ReusableTween?
                get {
                    Assert.IsTrue(sequence.isAlive);
                    Assert.IsTrue(current.IsCreated);
