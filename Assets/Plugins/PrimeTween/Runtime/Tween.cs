    ...
    public
        ...
        public float elapsedTimeTotal {
            get {
                ...
                if (tween.elapsedTimeTotal == float.MaxValue) {
                    return durationTotal;
                }
                return Mathf.Clamp(tween.elapsedTimeTotal - tween.waitDelay, 0f, durationTotal);
            }
        ...
        void setElapsedTimeTotal(float value) {
            ...
            if (isAlive) {
                ...
                if (value > durationTotalCached) {
                    tween.elapsedTimeTotal = durationTotalCached;
                }
            }
        }
        ...
        public float durationTotal {
            get {
                ...
                int cycles = tween.settings.cycles;
                if (cycles == -1) {
                    return float.PositiveInfinity;
                }
                Assert.AreNotEqual(0, cycles);
                return tween.cycleDuration * cycles;
            }
        }
        ...
        public void SetRemainingCycles(int cycles) {
            ...
            if (tween.tweenType == TweenAnimation.TweenType.Delay && tween.HasOnComplete) {
            ...
            if (cycles == -1) {
                if (tween.timeScale > 0f) {
                    ...
                    Debug.LogError($"'{nameof(SetRemainingCycles)}()' doesn't work with negative '{nameof(tween.timeScale)}' and infinite(-1) '{nameof(cycles)}'.");
                }
            } else {
                ...
                if (tween.timeScale > 0f) {
                    ...
                    tween.elapsedTimeTotal = targetCyclesDone * tween.cycleDuration + elapsedTime;
                    tween.cyclesDone = targetCyclesDone;
                    if (tween.settings.cycles < targetCyclesDone) {
                        tween.settings.cycles = targetCyclesDone + 1;
                    }
                }
            }
        }
