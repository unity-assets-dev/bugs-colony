...
public partial class Tests {
    ...
    public IEnumerator QuaternionDefaultValue() {
        ...
        }
        {
            ...
            t = Tween.Custom(def, def, 0.01f, delegate {
                numCallback++;
                var startVal = t.tween.startValue.quaternion;
                var endVal = t.tween.endValue.quaternion;
    ...
    public IEnumerator ShakeDuplication5() {
        ...
        Assert.AreEqual(iniPos, s1.tween.startValue.vector3);
        Assert.AreEqual(iniPos, s2.tween.startValue.vector3);
    }
    ...
    public IEnumerator FromToValues() {
        ...
        var data = new TweenSettings<float>(from, to, 0.01f);
        {
            ...
            Assert.AreEqual(from, t.tween.startValue.single);
            Assert.AreEqual(to, t.tween.endValue.single);
        }
        {
    ...
    public IEnumerator FromValueShouldNotChangeBetweenCycles() {
        ...
        while (tween.isAlive) {
            yield return null;
            Assert.AreEqual(origPos, tween.tween.startValue.vector3, "'From' should not change after a cycle. This can happen if tween resets startFromCurrent after a cycle.");
        }
        Object.Destroy(cube);
    }
    ...
    public IEnumerator RotationShakeEndVal() {
        ...
        var t = handle.tween;
        Assert.IsTrue(iniRot == t.startValue.quaternion);
        Assert.IsTrue(Quaternion.identity == t.endValue.quaternion);
